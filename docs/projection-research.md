...
Depth is currently an **image initialization**, not a trained ControlNet condition or a geometry guarantee. Fixed noise reduces random flicker but does not establish temporal coherence. The live ribbon continues moving while inference runs, so delay is visible. The implementation captures GPU depth every render frame and submits only the latest available capture when the model is free; it never queues every rendered frame behind slow inference.
